...
export default function Header ({
  ...
  const turnWarningsOnorOff = () => {
    if (warningsOnorOff == 'on') setWarningsOnorOff('off')
    else setWarningsOnorOff('on')
  ...
  return (
    <div className='flex justify-between items-center mb-6 p-4 bg-white shadow-lg rounded-lg'>
      ...
      <div className='space-x-2'>
        <Button variant='outline' onClick={event => saveGarden(event)}>
          Save Garden...
        </Button>
        <Button variant='outline' onClick={handleLoadClick}>
          Load Garden...
        </Button>
        <Button variant='destructive' onClick={event => clearGarden(event)}>
          Clear Garden
        </Button>
      </div>
    </div>
